...
def test_positive_case():
    ...
    filename = os.path.join(current_dir, "task3_cases/task3_test_case1.txt")
    """Checking that the minimum and maximum are found"""
    assert find_maximum_and_minimum(filename) == (6, 1)
    filename = os.path.join(current_dir, "task3_cases/task3_test_case2.txt")
    assert find_maximum_and_minimum(filename) == (0, 0)
    current_dir = os.path.dirname(__file__)
    filename = os.path.join(current_dir, "task3_cases/task3_test_case3.txt")
