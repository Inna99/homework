...
class ParsingMainPage:
    ...
    # def info_to_dict(self, name_dict):
    #     """adds information about companies from the main page to the dictionary"""
    #     company_name, growth_decline, current_value = self.get_growth_decline_and_current_value()
    #     current_value_rub = self.get_current_value_into_rubles(current_value)
    #     name_dict.update({company_name: {
    ...
    # @staticmethod
    # def get_current_value_into_rubles(usd):
    ...
    #     return curse_usd * float(usd)

    def get_growth_decline_and_current_value(self, name_dict):
...
class ParsingCompanyPage:
    ...
    def get_profit(self):
        ...
        try:
            current_tag = self.soup.find_all('div', {'class': 'snapshot__highlow'})[1].text.split()
            week_low = float(current_tag[0].replace(',', ''))
            week_high = float(current_tag[4].replace(',', ''))
        except IndexError as ex:
            logging.info('no week_low and week_higt')
            return 0, 0

        logging.debug(f"week_low={week_low}, week_high={week_high}")
        # current value
        current_tag = self.soup.find('div', {'class': 'snapshot'}).find_all('div', {'class': 'snapshot__data-item'})
        ...
        logging.debug(f"percent_sale_from_high={percent_sale_from_high}, percent_buy_from_low={percent_buy_from_low}")
